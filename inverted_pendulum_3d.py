    ...
    for j, sigma in enumerate(sigma_list):
        for sample_idx, N in enumerate(N_list):
            ...
            for exp in range(exp_num):
                ...
                ### Eliminate backward samples with that 3rd components out of the range [-1, 1]
                valid_mask = ((X_backward >= -1) & (X_backward <= 1)).all(dim=0).all(dim=1)
                X_backward = X_backward[:,valid_mask,:]
                # print('number of samples: ', X_backward.shape)
                N = X_backward.shape[1]
...
torch.save(X_pred, f'3dIP_traj_sigma{sigma}_epsilon{epsilon}_N{N}_T{T}.pt')
...
torch.save(X_backward, f'3dIP_Z_traj_sigma{sigma}_epsilon{epsilon}_N{N}_T{T}.pt')
